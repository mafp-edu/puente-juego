 ...
 * Muestra mensaje de El Profesor con typewriter, puntaje desglosado,
 * confeti pixel art, y botones para ver ranking o jugar de nuevo.
 *
 * Datos recibidos via init(data):
 *   data.puntajeFinal
 *   data.preguntasCorrectas
 *   data.enemigosDerrotados
 *   data.monedas
 *   data.tiempoSegundos
 */
...
export default class FinScene extends Phaser.Scene {
  ...
  init(data) {
    this._puntajeFinal       = data.puntajeFinal       || 0;
  ...
  create() {
    ...
    if (tieneCelebracion) {
      ...
      // Degradado de fusión imagen → panel
    ...
    // Dejar 116 px abajo para los 2 botones
    ...
    // ─ Portrait del Profesor (centrado) ─
    ...
    // ─ Texto del mensaje (debajo del portrait) ─
    const msgY   = portCY + portH / 2 + 22;
    const msgPad = 14;
    let nombre = 'Estudiante';
    try {
      const raw = localStorage.getItem('jugadorActual');
      if (raw) nombre = JSON.parse(raw).nombre || 'Estudiante';
    } catch {}

    const mensajeFinal =
      `Lo lograste, ${nombre}. Hoy no solo liberaste a tus compañeros — ` +
  ...
  // ── Botones (Phaser nativos, sin HTML) ──────────────────────────────────────
